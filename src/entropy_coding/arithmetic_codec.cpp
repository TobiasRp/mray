...
inline void Arithmetic_Codec::renorm_dec_interval(void)
{
    ...
    do
    {
        unsigned a = unsigned(base *= AC__OutputFactor);
        base -= double(a);
        value = (AC__OutputFactor * value - double(a)) +
                AC__LeastSignifBit * (256.0 * double(ac_pointer[0]) + double(ac_pointer[1]));
        ac_pointer += 2;
    } while ((length *= AC__OutputFactor) <= AC__MinLength);
}
...
void Arithmetic_Codec::put_bit(unsigned bit)
{
    ...
    else
    {
        ...
        length -= x;
        if (base >= 1.0)
            propagate_carry(); // check if carry bit
    }
...
unsigned Arithmetic_Codec::get_bit(void)
{
    ...
    else
    {
        ...
        length -= x;
        if (base >= 1.0)
        {                // decoder does not propagate carry
...
void Arithmetic_Codec::encode(unsigned bit, Adaptive_Bit_Model &M)
{
#ifdef _DEBUG
    if (mode != 1)
        AC_Error("encoder not initialized");
    ...
    if (bit == 0)
    { // update interval length and base
        length = x;
        ++M.bit_0_count;
    }
    else
...
void Arithmetic_Codec::encode(unsigned data, Static_Data_Model &M)
{
#ifdef _DEBUG
    if (mode != 1)
        AC_Error("encoder not initialized");
...
void Arithmetic_Codec::encode(unsigned data, Adaptive_Data_Model &M)
{
#ifdef _DEBUG
    if (mode != 1)
        AC_Error("encoder not initialized");
...
Arithmetic_Codec::Arithmetic_Codec(unsigned max_code_bytes, unsigned char *user_buffer)
{
    mode = buffer_size = 0;
    new_buffer = code_buffer = 0;
...
void Arithmetic_Codec::set_buffer(unsigned max_code_bytes, unsigned char *user_buffer)
{
...
unsigned Arithmetic_Codec::stop_encoder(void)
{
    ...
    if (b - a < 2)
    { // decide number of final byes based on interval length
...
void Static_Data_Model::set_distribution(unsigned number_of_symbols, const double probability[])
{
...
void Adaptive_Data_Model::set_distribution(unsigned number_of_symbols, const double probability[])
{
    set_alphabet(number_of_symbols);
    ...
    for (unsigned k = 0; k < data_symbols; k++)
    {
        symbol_count[k] = std::max(1u, static_cast<unsigned>(1000 * probability[k]));
        total_count += symbol_count[k] - 1;
    }
